...
use CC_RezdyAPI\Rezdy\Util\Validate;
...
abstract class BaseRequest
{
    ...
    public function attach($data)
    {
        ...
        if (is_array($data)) {
            ...
            if ($class == 'CC_RezdyAPI\Rezdy\Requests\Objects\PickupLocation') {
                $data->isValid();
            }
            ...
            if (array_key_exists($class, $this->setClassMap)) {
                ...
                if ($class == 'CC_RezdyAPI\Rezdy\Requests\Booking\Voucher') {
                    // Set the value
    ...
    {
        // 
        foreach (array_merge($this->requiredParams, $this->optionalParams) as $param => $type) {
        ...
        foreach ($this as $key => $value) {
            if ($this->requiredParams && !array_key_exists($key, array_merge($this->requiredParams, $this->optionalParams, $this->createMappingArray(), $this->restrictedParams ?? []))) {
                unset($this->$key);
            }
        }
    }
            ...
            switch ($lookup) {
                ...
                case 'priceOptionArray':
                    ...
                    foreach ($this->$param as $obj) {
                        if (get_class($obj) !== 'CC_RezdyAPI\Rezdy\Requests\Objects\SessionPriceOption') {
                            // Set an Error
